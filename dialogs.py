...
class Messagebox(Toplevel):
    def __init__(self, parent, title, details, icon, *, buttons) -> None:
        ...
        self.icon_label = Label(
            info_frame, image=icon, anchor="nw", background=self.color
        )
        if icon is not None:
        ...
        self.title_label.grid(
            row=0, column=1, sticky="nsew", padx=(12, 17), pady=(10, 8)
        )

        self.detail_label = Label(
            info_frame, text=details, anchor="nw", background=self.color
        )
        self.detail_label.grid(
            row=1, column=1, sticky="nsew", padx=(12, 17), pady=(5, 10)
        )

        self.button_frame = Frame(
            self.big_frame, padding=(22, 22, 12, 22), style="Dialog_buttons.TFrame"
        )
        ...
        self.x_coord = int(
            self.parent_width / 2 + self.parent_x - self.dialog_width / 2
        )
        self.y_coord = int(
            self.parent_height / 2 + self.parent_y - self.dialog_height / 2
        )
